...
### ORIENT (10 min)
5. Click through hot nodes. Read frontmatter. Hover-preview linked nodes.
6. Identify complete signal chains: Search node → Capture node → Validate node, all linked.
7. Flag infrastructure risks from `knowledge_base/gtm_signals/inboxkit/`

### ACT (20 min)
8. In Cursor (vault open): "Generate this week's MOC using weekly-moc-template"
9. Agent creates `00_foundation/weekly_updates/YYYY-MM-DD.md`
10. Open in Obsidian. Review. Edit "Human Decision Required" checkboxes.

### DEPOSIT (5 min)
11. `git add . && git commit -m "weekly: YYYY-MM-DD — [one-line summary]"`
12. Weekly note stays in graph. Next week's agent sees it in `co-access` queries.
13. Copy note body → paste into email → send to Josh + investors.
