---
name: WEEKLY_HEYREACH_EVIDENCE_2026_05_04
description: HeyReach outbound metrics snapshot for Wave 1 launch week ending 2026-05-04
domain: business
node_type: gtm_signal
status: emergent
date: "2026-05-04"
evidence_date: 2026-05-04
tags:
  - heyreach
  - weekly-evidence
  - outbound
  - wave1
topics:
  - gtm-motion
related_concepts:
  - "[[WEEKLY_MOC_2026_05_04]]"
  - "[[WEEKLY_MOC_GRAPH_RITUAL]]"
  - "[[wave1a_heyreach_copy]]"
  - "[[OUTREACH_WAVE_STRUCTURE]]"
  - "[[WAVE_1_SCORING_FRAMEWORK]]"
  - "[[TECH_STACK_OUTBOUND_INFRASTRUCTURE]]"
source:
  type: tool_export
  system: heyreach-mcp
  date: "2026-05-04"
---

# HeyReach Week of Apr 28 - May 4

[VERIFIED: Operator pull attributed to heyreach-mcp export dated 2026-05-04 per this node's source frontmatter]

| Metric | Count |
|--------|-------|
| Active campaigns | 3 |
| Total leads loaded | 1,219 |
| Connection requests in progress | 104 |
| Pending sends | 1,115 |
| Campaign replies | 0 |
| Manual conversations (non-campaign) | 4 |
| Positive manual signals | 1 |
| Neutral manual signals | 3 |
| Sender health | 1/1 healthy |

## Campaign Breakdown

| Campaign | List | Leads | In Progress | Pending |
|----------|------|-------|-------------|---------|
| Wave1-Baseline-9Subtier | Baseline | 270 | 35 | 235 |
| Wave1-OperationalOwner | 2A ACO | 359 | 35 | 324 |
| Wave1-ClinicalChampion | 1C Provider | 590 | 34 | 556 |

## Interpretation

Campaigns launched Monday May 4. Zero replies expected on Day 1. 104 connection requests active. Signal data will appear by Thursday May 8.

Links: [[wave1a_heyreach_copy]], [[OUTREACH_WAVE_STRUCTURE]], [[WAVE_1_SCORING_FRAMEWORK]]

## What This Proves (Strategy Links)

This evidence validates the following strategic nodes:

- [[OUTREACH_WAVE_STRUCTURE]] — 3 campaigns = Wave 1 structure exercised at scale
- [[WAVE_1_SCORING_FRAMEWORK]] — 1,219 leads loaded and scored against pilot feasibility
- [[PERSONA_TITLE_DICTIONARY_BY_SUBTIER]] — 2A ACO / 1C Provider / Baseline personas targeted with correct title strings
- [[BUYER_PERSONA_OPERATIONAL_BUYER]] — 359 leads routed to operational owner (2A ACO)
- [[BUYER_PERSONA_CLINICAL_CHAMPION]] — 590 leads routed to clinical champion (1C Provider)
- [[wave1a_heyreach_copy]] — Copy variants loaded into live campaigns
- [[COCM_CONFIDENCE_GRADE_RULES]] — Grade-aware outreach ready for CoCM revenue claims
- [[WEEKLY_MOC_2026_05_04]] — Weekly hub citing this evidence
- [[WEEKLY_MOC_GRAPH_RITUAL]] — Ritual that produced this cohesion
